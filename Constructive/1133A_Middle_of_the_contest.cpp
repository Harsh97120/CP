...
void task()
{
    ...
    // print("Ans: \n");
    int total_miutes = (h2 - h1)*60 + (60-m1+m2);
    total_miutes /= 2 ; 

    print("Total_min: " , total_miutes);

    int hr = total_miutes/60;
    int min = total_miutes%60;

    int ans_hr = (h1+hr)+((m1 + min)%60 == 0 ? 1:0);
    int ans_min = (m1 + min)%60;
