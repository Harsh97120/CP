...
void task()
{
    ...
    // We have to choose from middle since if we
    // choose median from right side then another 
    // median will come from left side so difference 
    // will increase and vice -versa.
    

    print(abs(arr[(2*n - 1)/2] - arr[((2*n - 1)/2)+1]) , "\n");
