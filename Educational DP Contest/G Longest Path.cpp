...
int minimum(vector<long long> &arr){ return *min_element(arr.begin(), arr.end()); }
int maximum(vector<long long> &arr){ return *max_element(arr.begin(), arr.end()); }
...
ll dfs(ll node , ll &cnt , ll &parent ,
         llVec &dp , boolVec &vis , 
         llGrid &graph)
{
    ll ans = 0 ; 
    ...
    {
        ll path = 1 + dfs(it , cnt , parent , dp , vis , graph);
        ans = max(ans , path);
    }
...
void task()
{
    ...
    llloop(i , 1 , n+1)
    {
        ans = max(ans, dfs(i, cnt , i,dp,vis,graph));
    }
