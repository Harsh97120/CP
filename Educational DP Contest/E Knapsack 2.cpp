...
ll inf = 1e18 ;
ll f(ll i , ll p , ll n, llGrid &arr , llGrid &dp)
{
    if(p == 0) return 0 ;
    if(i == n)
    {
    ...
    ll nottake = f(i+1 , p , n, arr , dp);
    ll take = 1e18 ; 
    
    if(arr[i][1] <= p)
    {
        take = arr[i][0]+ f(i+1 , p - arr[i][1], n, arr , dp);
    }

    return dp[i][p] = min(nottake , take);
