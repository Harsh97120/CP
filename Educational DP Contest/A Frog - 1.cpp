...
ll f(ll i ,ll n, llVec &arr , llVec &dp)
{
    if(i == n - 1)return 0 ; 

    if(dp[i] != -1) return dp[i];

    ll j1 = abs(arr[i] - arr[i+1]) + f(i+1 , n, arr , dp);
    ll j2 = 1e9 + 1 ; 

    if(i+2 < n)
    {
        j2 = abs(arr[i] - arr[i+2]) + f(i+2 , n, arr , dp);
    }
...
void task()
{
    ...
    dp[n-2] = abs(arr[n-2] - arr[n-1]) + dp[n-1];

    for(ll i=n-3 ;i >= 0; --i)
    {
        ll j1 = abs(arr[i] - arr[i+1]) + dp[i+1];
        ll j2 = abs(arr[i] - arr[i+2]) + dp[i+2];
