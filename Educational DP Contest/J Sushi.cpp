...
void task()
{
    ...
    function <double(ll , ll , ll)> dfs = [&](ll i , ll j , ll k)
    {
        ...
        if(i) dp[i][j][k] += dfs(i-1 , j , k) * i/(i+j+k);
        if(j) dp[i][j][k] += dfs(i+1 , j-1 , k) * j/(i+j+k);
        if(k) dp[i][j][k] += dfs(i , j+1 , k-1) * k/(i+j+k);
    ...
    cout << fixed << setprecision(14) <<dfs(no1 , no2 , no3) << "\n";
}
