...
ll f(ll choice , ll i , ll n , llGrid &arr , llGrid &dp)
{
    if(i == n)
    {
        return 0;
    ...
    llloop(ind , 0 , 3)
    {
        ...
        ll t = arr[i][ind] + f(ind,i+1 , n , arr , dp);
        score = max(score , t);
    }

    return dp[i][choice] = score ;
}
...
void task()
{
    ...
    // llGrid dp(n , llVec (3 , -1));
    // ans = max(f(0 , 0 ,n, arr , dp) , f(1 , 0 ,n, arr , dp));
    // ans = max(ans , f(2 , 0 , n , arr , dp));

    llGrid dp(n+1 , llVec (3 , 0));

    for(ll i=n-1 ;i >= 0; --i)
    {

        for(ll choice = 0 ;choice <= 2; ++choice)
        {
            ll score = 0 ; 
