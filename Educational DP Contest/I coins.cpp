...
double f(ll i , ll tails, ll n , ll maxtails ,  vector <double> &arr , vector <vector <double>> &dp)
{
    if(i == n)
    {
        return 1.0; 
    ...
    double ans = 0 ; 
    double step1 = arr[i]*f(i+1 , tails , n , maxtails , arr , dp);
    double step2 = 0 ;

    if(tails < maxtails)
    {
        step2 = (1 - arr[i])*f(i+1 , tails+1 , n , maxtails , arr , dp);
    }

    ans = step1 + step2 ;
    

    return dp[i][tails] = ans ; 
}
...
void task()
{
    ...
    double ans = 0 ; 
    ll maxtails = (n/2) - (n%2 == 0);
    // vector <vector <double>> dp();
    vector <vector <double>> dp(n , vector <double> (maxtails+1 , -1));
    ans = f(0 , 0 , n , maxtails , arr , dp);
