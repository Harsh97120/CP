...
ll f(ll i , ll w , ll n , llGrid &arr , llGrid &dp)
{
    if(i==n-1)
    {
        if(arr[i][0] <= w) return arr[i][1];
        return 0 ;
    }

    if(dp[i][w] != -1) return dp[i][w];

    ll nottake = f(i+1 , w , n , arr , dp);
    ll take = 0 ; 

    if(arr[i][0] <= w)
    {
        take = arr[i][1] + f(i+1 , w - arr[i][0] , n , arr , dp);
    }
...
ll f(ll i , ll w , ll n , llGrid &arr , unordered_map <ll ,unordered_map <ll ,ll>> &dp)
{
    if(i==n-1)
    {
        if(arr[i][0] <= w) return arr[i][1];
    ...
    if(dp.find(i) != dp.end() && dp[i].find(w) != dp[i].end()) return dp[i][w];
...
void task()
{
    ...
    // llGrid dp(n , llVec (w+1 , -1));
    // ll ans = f(0 , W , n , arr , dp);
    // print(ans , "\n");
